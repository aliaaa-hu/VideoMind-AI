...
public class QdrantVectorStore {
    ...
    public QdrantVectorStore(@Value("${vector.qdrant.enabled:true}") boolean enabled,
                             ...
                             @Value("${vector.qdrant.embedding-version:bge-m3-v1}") String embeddingVersion,
                             @Value("${vector.qdrant.chunk-strategy-version:five-minute-summary-v1}") String chunkStrategyVersion) {
        ...
        this.embeddingVersion = requireVersion("embedding version", embeddingVersion);
        this.chunkStrategyVersion = requireVersion("chunk strategy version", chunkStrategyVersion);
    }
    ...
    public void upsert(Long mediaId, List<VideoChunk> chunks) {
        ...
        try {
            ...
            for (VideoChunk chunk : vectorized) {
                ...
                payload.put("embeddingVersion", embeddingVersion);
                payload.put("chunkStrategyVersion", chunkStrategyVersion);
            ...
            throw new IllegalStateException("Failed to write segment vectors to Qdrant", e);
        }
    }
    ...
    public List<VectorHit> search(Long mediaId, List<Double> queryEmbedding, int limit) {
        ...
        try {
            ...
            filter.put("must", List.of(
                    condition,
                    versionCondition("embeddingVersion", embeddingVersion),
                    versionCondition("chunkStrategyVersion", chunkStrategyVersion)));
            ...
            collectionReady.set(false);
            throw new IllegalStateException("Qdrant semantic retrieval failed", e);
        }
    }
    ...
    private String pointId(Long mediaId, VideoChunk chunk) {
        String source = mediaId + ":" + chunk.startTime() + ":" + chunk.endTime()
                + ":" + embeddingVersion + ":" + chunkStrategyVersion;
        return UUID.nameUUIDFromBytes(source.getBytes(StandardCharsets.UTF_8)).toString();
    }

    private JSONObject versionCondition(String key, String value) {
        JSONObject match = new JSONObject();
    ...
    private String requireVersion(String name, String value) {
        if (value == null || !value.matches("[A-Za-z0-9._-]{1,96}")) {
            throw new IllegalArgumentException("Qdrant " + name + " is invalid");
        }
        return value;
    }
