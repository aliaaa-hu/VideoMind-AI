...
public class AudioExportService {
    ...
    public Path exportMp3(MediaFile mediaFile) {
        ...
        if (inputPath == null || inputPath.isBlank()
            ...
            throw new NoSuchElementException("Video source file does not exist");
        }

        Path outputPath = null;
        Process process = null;
        try {
            outputPath = Files.createTempFile("videomind-audio-", ".mp3");
            ...
            if (!process.waitFor(CONVERSION_TIMEOUT_MINUTES, TimeUnit.MINUTES)) {
                ...
                throw new IllegalStateException("Audio conversion timed out");
            }
            if (process.exitValue() != 0) throw new IllegalStateException("Audio conversion failed");
            return outputPath;
        } catch (Exception e) {
