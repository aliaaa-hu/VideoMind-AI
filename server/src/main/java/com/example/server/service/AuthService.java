...
public class AuthService {
    ...
    public AuthResponse register(AuthRequest request) {
        ...
        if (username == null || password == null || password.length() < 8
            ...
            return response(400, "Username must be 3-32 letters, numbers, or underscores; password must be 8-128 characters.", null, null);
        }

        String nickname = normalizeNickname(request.nickname());
        if (nickname == null) {
            return response(400, "Nickname must not exceed 50 characters.", null, null);
        }

        QueryWrapper<User> query = new QueryWrapper<>();
        query.eq("username", username);
        if (userMapper.selectCount(query) > 0) {
            return response(409, "This username already exists.", null, null);
        }

        User user = new User();
        user.setUsername(username);
        user.setPassword(hashPassword(password));
        user.setNickname(nickname.isBlank() ? "User" + System.currentTimeMillis() : nickname);
        user.setRole("USER");
        try {
        ...
        return response(200, "Registration successful.", userView(user), null);
    }

    public AuthResponse login(AuthRequest request) {
        String username = normalizeUsername(request.username());
        if (username == null || request.password() == null || request.password().isBlank()) {
            return response(400, "Username and password are required.", null, null);
        }
        if (!loginAttemptAllowed(username)) {
            return response(429, "Too many sign-in attempts. Please try again later.", null, null);
        }
        ...
        if (user == null || !passwordMatches(request.password(), user.getPassword())) {
            ...
            return response(401, "Invalid username or password.", null, null);
        }
        ...
        return response(200, "Sign-in successful.", userView(user), token);
    }

    public void requireAdmin(Long userId) {
        User user = userMapper.selectById(userId);
        if (user == null || !"ADMIN".equals(user.getRole())) {
            throw new SecurityException("Only administrators can manage failed tasks.");
        }
    }
    ...
    public Long resolveUser(String authorization) {
        ...
        if (userId == null) throw new SecurityException("Your sign-in session has expired.");
    ...
    private String bearerToken(String authorization) {
        if (authorization == null || !authorization.startsWith("Bearer ")) {
            throw new SecurityException("Please sign in first.");
        }
        String token = authorization.substring("Bearer ".length()).trim();
        if (token.length() < 32) throw new SecurityException("Invalid sign-in credential.");
        return token;
    }
    ...
    private String sessionKey(String token) {
        try {
            ...
            throw new IllegalStateException("Failed to create session digest", e);
        }
    }
