...
public class MediaService {
    ...
    public String calculateMd5(String value) {
        ...
        try {
            return HexFormat.of().formatHex(md5Digest().digest(value.getBytes(java.nio.charset.StandardCharsets.UTF_8)));
    ...
    public MediaFile saveUploadedMedia(String filename, String fileUrl, Long userId, String md5) {
        ...
        mediaFile.setIngestionMode("FULL_VIDEO");
    ...
    public MediaFile saveSubtitleFirstMedia(String filename,
                                             String sourceUrl,
                                             Long userId,
                                             String contentHash,
                                             Long sourceDurationMs) {
        if (sourceUrl == null || sourceUrl.isBlank()) throw new IllegalArgumentException("Source URL is required.");
        MediaFile mediaFile = new MediaFile();
        mediaFile.setFilename(normalizeVideoFilename(filename));
        // Keep the source in filePath for backwards-compatible context ownership; playback checks the mode.
        mediaFile.setFilePath(sourceUrl);
        mediaFile.setSourceUrl(sourceUrl);
        mediaFile.setIngestionMode("SUBTITLE_FIRST");
        mediaFile.setSourceDurationMs(sourceDurationMs);
        mediaFile.setStatus("COMPLETED");
        mediaFile.setUploadTime(LocalDateTime.now());
        mediaFile.setUserId(userId);
    ...
    public MediaFile promoteToFullVideo(MediaFile mediaFile, String fileUrl, String contentHash) {
        if (mediaFile == null) throw new IllegalArgumentException("Media is required.");
        mediaFile.setFilePath(fileUrl);
        mediaFile.setIngestionMode("FULL_VIDEO");
        mediaFile.setContentHash(contentHash);
    ...
    public MediaFile attachRemoteSource(MediaFile mediaFile, String sourceUrl, Long sourceDurationMs) {
        if (mediaFile == null) throw new IllegalArgumentException("Media is required.");
        mediaFile.setSourceUrl(sourceUrl);
        mediaFile.setSourceDurationMs(sourceDurationMs);
        mediaFileMapper.updateById(mediaFile);
    ...
    public List<MediaFile> listByUser(Long userId) {
        ...
        List<MediaFile> mediaFiles = mediaFileMapper.selectList(
                query.select("id", "filename", "status", "cover_url", "upload_time", "ingestion_mode", "source_duration_ms")
                        .eq("user_id", userId)
                        .orderByDesc("id"));
        try {
    ...
    public void deleteOwnedMedia(Long mediaId, Long userId) {
        ...
        if (minioUtils.isManagedFile(mediaFile.getFilePath())) {
            try {
    ...
    public String normalizeVideoFilename(String filename) {
        ...
        if (normalized.isBlank() || normalized.length() > 255) {
            throw new IllegalArgumentException("Video filename is invalid or too long.");
        }
        String suffix = fileSuffix(normalized).toLowerCase(java.util.Locale.ROOT);
        if (!VIDEO_SUFFIXES.contains(suffix)) {
            throw new IllegalArgumentException("Only MP4, MOV, MKV, AVI, WEBM, and M4V videos are supported.");
        }
        return normalized;
    }
    ...
    public MediaFile requireOwnedMedia(Long mediaId, Long userId) {
        MediaFile mediaFile = mediaFileMapper.selectById(mediaId);
        if (mediaFile == null) throw new NoSuchElementException("File does not exist.");
        if (!Objects.equals(mediaFile.getUserId(), userId)) {
            throw new SecurityException("You do not have access to this file.");
        }
        return mediaFile;
    }
