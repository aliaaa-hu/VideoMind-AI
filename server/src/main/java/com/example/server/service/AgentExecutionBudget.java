...
public final class AgentExecutionBudget {
    ...
    public static Scope open(long maxDurationMs) {
        if (maxDurationMs < 1) throw new IllegalArgumentException("Agent execution time budget must be greater than 0");
    ...
    public static long remainingMillis() {
        ...
        if (remainingNanos <= 0) throw new DeadlineExceededException("Agent execution time budget has been exhausted");
    ...
    public static void check(String stage) {
        try {
            ...
            throw new DeadlineExceededException("Stopped after " + stage + ": " + e.getMessage());
        }
    }
