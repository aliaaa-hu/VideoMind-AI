...
if ! java -version >/dev/null 2>&1 && command -v brew >/dev/null 2>&1; then
  jdk_bin="$(brew --prefix openjdk@21 2>/dev/null || true)/bin"
...
if [[ -z "${SILICONFLOW_API_KEY:-}" ]]; then
  echo "SILICONFLOW_API_KEY is not set; infrastructure will start, but AI-powered features remain unavailable."
fi

if [[ ! -d mysql/data/mysql && "${DB_USERNAME:-}" != "${MYSQL_APP_USER:-videomind}" ]]; then
...
curl --fail --silent --show-error --retry 20 --retry-connrefused --retry-delay 1 \
  "${MINIO_ENDPOINT%/}/minio/health/live" >/dev/null
